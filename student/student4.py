...
# BBA-2 variant
# Designed to perform best in low buffer live environments
# by Zane Middaugh
...
def chunkMapNonlinear(curr_buf,max_buf,rate_max,rate_min,map_range):
	...
	if curr_buf <= a:
		...
		# derive nonlinear function to optimize live choices
		x = curr_buf / max_buf
		sig = (0.4 * (1 / (1 + exp(-10 * (x - 0.5))))) + 0.6 * (x ** 2)
		fb =  sig * (rate_max - rate_min)

	return fb


def BBAlive(rate_prev,curr_buf,resv_size,cush_size,map_range,max_buf,m):

	vid_rates = range(3)
	max_rate = max(vid_rates)
	min_rate = min(vid_rates)
	rmap = chunkMapNonlinear(curr_buf,max_buf,max_rate,min_rate,map_range)
	...
	if curr_buf - incoming_rate_penalty >= 0.7 * curr_buf:
...
def student_entrypoint(client_message: ClientMessage):
	...
	# run BBA-live
	res = BBAlive(past_qual,c.buffer_seconds_until_empty,map_range[0],map_range[1]-map_range[0],map_range,c.buffer_max_size,c)
...
# uncomment below code to see graph
	...
	return idx  # return BBA live selected bitrate